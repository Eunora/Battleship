...
public class AIHardPlayer : AIPlayer
{
    ...
    protected class Target
    {
        ...
        /* summary & value & returns : The target shot at*/
        ...
        /* summary & value & returns  :The source that added this location as a target. */
    ...
    /* summary : Private enumarator for AI states. currently there are two states, the AI can be searching for a ship, or if it has found a ship it will target the same ship*/
    ...
    /* param (name=row & column) : the row/coloum that will be shot a*/
    ...
    protected override void GenerateCoords(ref int row, ref int column)
    {
        do
        {
            ...
            /* summary : check which state the AI is in and uppon that choose which coordinate generation method will be used.*/
    ...
    /* param (name=row & column) : row/column generated around the hit tile*/
    ...
    /* param (name=row & column) : the generated row/column*/
    ...
    /* summary : ProcessShot is able to process each shot that is made and call the right methods belonging to that shot. For example, if its a miss = do nothing, if it's a hit = process that hit location*/
    /* param (name=row & column) : the row/column that was shot at */
    /* param (name=result) : the result from that hit */
    ...
    /* summary : ProcessDetroy is able to process the destroyed ships targets and remove _LastHit targets.It will also call RemoveShotsAround to remove targets that it was going to shoot at*/
    /* param (name=row & column &ship) : the row/column that was shot at and destroyed */
    private void ProcessDestroy(int row, int col, Ship ship)
    {
        ...
        for (i = 1; i <= ship.Hits - 1; i++)
        {
            if (!foundOriginal)
            {
                ...
                /* summary :  Source is nnothing if the ship was originally hit in the middle. This then searched forward, rather than backward through the list of targets*/
                if (source == null)
                {
             ...
             /* summary : find the source in _LastHit */
    ...
    /* summary : RemoveShotsAround will remove targets that belong to the destroyed ship by checking if the source of the targets belong to the destroyed ship. If they don't put them on a new stack. Then clear the targets stack and move all the targets that still need to be shot at back onto the targets stack*/
    /* param (name=toRemove) : -- */
    private void RemoveShotsAround(Location toRemove)
    {
        Stack<Target> newStack = new Stack<Target>();  // create a new stack

        // summary : check all targets in the _Targets stack
        foreach (Target t in _Targets)
        {

            // sumamry : if the source of the target does not belong to the destroyed ship put them on the newStack
        ...
        // summary : for all the targets in the newStack, move them back onto the _Targets stack
        foreach (Target t in newStack)
            _Targets.Push(t);

        // summary : if the _Targets stack is 0 then change the AI's state back to searching
        if (_Targets.Count == 0)
            _CurrentState = AIStates.Searching;
    }
    ...
    private void ProcessHit(int row, int col)
    {
        ...
        {
            // sumamry : either targetting or hitting... both are the same here
    ...
    // summary : ReOrderTargets will optimise the targeting by re-orderin the stack that the targets are in. By putting the most important targets at the top they are the ones that will be shot at first.
    private void ReOrderTargets()
    {

        // summary : if the ship is lying on the same row, call MoveToTopOfStack to optimise on the row
        if (_CurrentTarget.SameRow)
            MoveToTopOfStack(_CurrentTarget.ShotAt.Row, -1);
        else if (_CurrentTarget.SameColumn)
            // summary : else if the ship is lying on the same column, call MoveToTopOfStack to optimise on the column
            MoveToTopOfStack(-1, _CurrentTarget.ShotAt.Column);
    }

    // summary : MoveToTopOfStack will re-order the stack by checkin the coordinates of each target If they have the right column or row values it will be moved to the _Match stack else put it on the _NoMatch stack. Then move all the targets from the _NoMatch stack back on the _Targets stack, these will be at the bottom making them less important. The move all the targets from the _Match stack on the _Targets stack, these will be on the top and will there for be shot at first
    /* param (name=row & column) : the row/column of the optimisation */
    ...
    /* param (name=row & column) : the row/column of the targets location */
